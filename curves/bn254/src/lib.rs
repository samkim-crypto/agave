...
const FIELD_LEN: usize = ALT_BN128_FIELD_SIZE;
const G1_LEN: usize = FIELD_LEN * 2;
const G2_LEN: usize = FIELD_LEN * 4;

// The base field (field characteristic) modulus `p` as big-endian binary bytes
// p = 0x2523648240000001BA344D80000000086121000000000013A700000000000013
const P_BYTES: [u8; FIELD_LEN] = [
    37, 35, 100, 130, 64, 0, 0, 1, 186, 52, 77, 128, 0, 0, 0, 8, 97, 33, 0, 0, 0, 0, 0, 19, 167, 0,
    0, 0, 0, 0, 0, 19,
];

/// The BN254 (BN128) group element in G1 as a POD type.
///
/// A group element in G1 consists of two field elements `(x, y)`. A `PodG1` type expects
/// a group element to be byte-encoded as `[le(x), le(y)]` where `le(..)` is a little-endian
/// encoding of the input field element, which follows the `ark-bn254` implementation. Note that
/// this differs from the EIP-197 standard, which specifies that the field elements are encoded as
/// big-endian.
///
/// The Solana syscalls still expect the inputs to be encoded as specified in EIP-197. The type
/// `PodG1` is an intermediate type that facilitates the translation between the EIP-197 encoding
/// and the arkworks implementation encoding.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Pod, Zeroable)]
#[repr(transparent)]
pub struct PodG1(pub(crate) [u8; G1_LEN]);

impl PodG1 {
    /// Takes in an EIP-197 (big-endian) byte encoding of a group element in G1 and constructs a `PodG1`
    /// struct that encodes the same bytes in little-endian. In the process, it
    /// checks that the encoded big-number is strictly less than the
    /// base field modulus (the field characteristic).
    pub fn new(be_bytes: &[u8]) -> Result<Self, AltBn128Error> {
        if be_bytes.len() != G1_LEN {
            return Err(AltBn128Error::SliceOutOfBounds);
        }
        let mut pod_bytes = [0u8; G1_LEN];
        check_field_reverse_copy(&be_bytes[..FIELD_LEN], &mut pod_bytes[..FIELD_LEN])?;
        check_field_reverse_copy(&be_bytes[FIELD_LEN..], &mut pod_bytes[FIELD_LEN..])?;
        Ok(Self(pod_bytes))
    }
}
...
/// element itself is a degree 1 polynomial `x = x0 + x1*X`, `y = y0 + y1*X`. The EIP197 standard
...
/// element. Notably, in addition to the difference in the big-endian vs. little-endian
/// encodings of field elements, the order of the polynomial field coefficients
/// `x0`, `x1`, `y0`, and `y1` also differ.
///
/// The Solana syscalls still expect the inputs to be encoded as specified in EIP-197. The type
...
#[derive(Clone, Copy, Debug, PartialEq, Eq, Pod, Zeroable)]
#[repr(transparent)]
pub struct PodG2(pub(crate) [u8; G2_LEN]);

impl PodG2 {
    /// Takes in an EIP-197 (big-endian) byte encoding of a group element in G2 and constructs a `PodG2`
    /// struct that encodes the same bytes in little-endian. In the process, it
    /// checks that the encoded big-number is strictly less than the
    /// base field modulus (the field characteristic).
    ///
    /// This constructor expects the EIP197 byte representation of a G2 element and converts it
    /// into the arkworks byte representation.
    pub fn new(be_bytes: &[u8]) -> Result<Self, AltBn128Error> {
        if be_bytes.len() != G2_LEN {
            return Err(AltBn128Error::SliceOutOfBounds);
        }
        let mut pod_bytes = [0u8; G2_LEN];

        // note the cross order
        const SOURCE_X1_INDEX: usize = 0;
        const SOURCE_X0_INDEX: usize = SOURCE_X1_INDEX.saturating_add(FIELD_LEN);
        const SOURCE_Y1_INDEX: usize = SOURCE_X0_INDEX.saturating_add(FIELD_LEN);
        const SOURCE_Y0_INDEX: usize = SOURCE_Y1_INDEX.saturating_add(FIELD_LEN);

        const TARGET_X0_INDEX: usize = 0;
        const TARGET_X1_INDEX: usize = TARGET_X0_INDEX.saturating_add(FIELD_LEN);
        const TARGET_Y0_INDEX: usize = TARGET_X1_INDEX.saturating_add(FIELD_LEN);
        const TARGET_Y1_INDEX: usize = TARGET_Y0_INDEX.saturating_add(FIELD_LEN);

        check_field_reverse_copy(
            &be_bytes[SOURCE_X1_INDEX..SOURCE_X1_INDEX.saturating_add(FIELD_LEN)],
            &mut pod_bytes[TARGET_X1_INDEX..TARGET_X1_INDEX.saturating_add(FIELD_LEN)],
        )?;
        check_field_reverse_copy(
            &be_bytes[SOURCE_X0_INDEX..SOURCE_X0_INDEX.saturating_add(FIELD_LEN)],
            &mut pod_bytes[TARGET_X0_INDEX..TARGET_X0_INDEX.saturating_add(FIELD_LEN)],
        )?;
        check_field_reverse_copy(
            &be_bytes[SOURCE_Y1_INDEX..SOURCE_Y1_INDEX.saturating_add(FIELD_LEN)],
            &mut pod_bytes[TARGET_Y1_INDEX..TARGET_Y1_INDEX.saturating_add(FIELD_LEN)],
        )?;
        check_field_reverse_copy(
            &be_bytes[SOURCE_Y0_INDEX..SOURCE_Y0_INDEX.saturating_add(FIELD_LEN)],
            &mut pod_bytes[TARGET_Y0_INDEX..TARGET_Y0_INDEX.saturating_add(FIELD_LEN)],
        )?;
        Ok(Self(pod_bytes))
    }
}

/// Takes in a `source` byte slice, which encodes a 32-byte number in big-endian and copies the
/// bytes into a `destination` byte slice in reverse order thereby encoding the same number in
/// little-endian. At the same time, it verifies that the encoded 32-byte number is strictly less than
/// the base field modulus (field characteristic). The function is designed specifically to
/// perform the copying and the strict comparison in a single loop.
fn check_field_reverse_copy(source: &[u8], destination: &mut [u8]) -> Result<(), AltBn128Error> {
    if source.len() != ALT_BN128_FIELD_SIZE || destination.len() != ALT_BN128_FIELD_SIZE {
        return Err(AltBn128Error::SliceOutOfBounds);
    }

    // true if a previously compared byte is strictly less than the corresponding prime byte
    let mut strict_condition = false;
    let mut destination_index = ALT_BN128_FIELD_SIZE;
    for i in 0..ALT_BN128_FIELD_SIZE {
        // if a previously compared byte is strictly less, then skip subsequent comparisons
        if !strict_condition {
            match source[i].cmp(&P_BYTES[i]) {
                Ordering::Less => {
        ...
        destination_index = destination_index
            .checked_sub(1)
            .ok_or(AltBn128Error::SliceOutOfBounds)?; // this error should never be triggered
        destination[destination_index] = source[i];
    }

    // if none of the bytes are strictly less than the prime bytes and the logic does not
    // return with an error above, then the encoded number is equal to the prime
    if !strict_condition {
        return Err(AltBn128Error::GroupError);
    }
    Ok(())
...
mod target_arch {
    ...
    /// Adds two group elements in the curve BN254 (BN128).
    ///
    /// This function is identical to `alt_bn128_addition` except that it
    /// returns an error if any of the field elements of each curve point
    /// exceeds the field modulus.
    pub fn alt_bn128_addition_checked(input: &[u8]) -> Result<Vec<u8>, AltBn128Error> {
        if input.len() > ALT_BN128_ADDITION_INPUT_LEN {
            return Err(AltBn128Error::InvalidInputData);
        }
        ...
        let p: G1 = PodG1::new(&input[..G1_LEN])?.try_into()?;
        let q: G1 = PodG1::new(&input[G1_LEN..])?.try_into()?;
    ...
    /// Multiplies a group element in the curve BN254 (BN128) by a scalar
    /// number.
    ///
    /// This function is identical to `alt_bn128_multiplication` except that it
    /// returns an error if any of the field elements of the curve point
    /// exceeds the field modulus.
    pub fn alt_bn128_multiplication_checked(input: &[u8]) -> Result<Vec<u8>, AltBn128Error> {
        if input.len() > ALT_BN128_MULTIPLICATION_INPUT_LEN {
            return Err(AltBn128Error::InvalidInputData);
        }
    ...
    /// Computes the pairings of group elements in the curve BN254 (BN128).
    ///
    /// This function is identical to `alt_bn128_pairing` except that it
    /// returns an error if any of the field elements of the curve points
    /// exceeds the field modulus.
    pub fn alt_bn128_pairing_checked(input: &[u8]) -> Result<Vec<u8>, AltBn128Error> {
        ...
        for _ in 0..ele_len {
            ...
            let p_bytes_end = p_bytes_start.saturating_add(G1_LEN);
            let q_bytes_start = p_bytes_end;
            let q_bytes_end = q_bytes_start.saturating_add(G2_LEN);

            vec_pairs.push((
                PodG1::new(&input[p_bytes_start..p_bytes_end])?.try_into()?,
                PodG2::new(&input[q_bytes_start..q_bytes_end])?.try_into()?,
            ));
...
mod tests {
    ...
    #[test]
    fn alt_bn128_addition_unchecked_test() {
    ...
    #[test]
    fn alt_bn128_addition_checked_test() {
        use serde_derive::Deserialize;

        let success_test_data = r#"[
        {
            "Input": "",
            "Expected": "2243525c5efd4b9c3d3c45ac0ca3fe4dd85e830a4ce6b65fa1eeaee202839703301d1d33be6da8e509df21cc35964723180eed7532537db9ae5e7d48f195c915",
            "Name": "chfast1",
        ...
        let test_cases: Vec<SuccessTestCase> = serde_json::from_str(success_test_data).unwrap();

        test_cases.iter().for_each(|test| {
            let input = array_bytes::hex2bytes_unchecked(&test.input);
            let result = alt_bn128_addition_checked(&input);
            assert!(result.is_ok());
        ...
        let fail_test_data = r#"[
        {
            ...
            "Input": "",
            "Name": "exactly prime",
            "Gas": 150,
            "NoBenchmark": false
        }
        ...
        let test_cases: Vec<FailTestCase> = serde_json::from_str(fail_test_data).unwrap();

        test_cases.iter().for_each(|test| {
            let input = array_bytes::hex2bytes_unchecked(&test.input);
            let result = alt_bn128_addition_checked(&input).unwrap_err();
            assert_eq!(result, AltBn128Error::GroupError);
        });
    }

    #[test]
    fn alt_bn128_multiplication_unchecked_test() {
        use serde_derive::Deserialize;
    ...
    #[test]
    fn alt_bn128_multiplication_checked_test() {
        use serde_derive::Deserialize;
        ...
        test_cases.iter().for_each(|test| {
            let input = array_bytes::hex2bytes_unchecked(&test.input);
            let result = alt_bn128_multiplication_checked(&input);
            assert!(result.is_ok());
            let expected = array_bytes::hex2bytes_unchecked(&test.expected);
            assert_eq!(result.unwrap(), expected);
        });
        ...
        let fail_test_data = r#"[
        {
            ...
            "Input": "2523648240000001BA344D80000000086121000000000013e849a8a7fa163fa901e0559bacb160664764a357af8a9fe70baa9258e0b959273ffc5718c6d4cc7c039730ea8dff1254c0fee9c0ea777d29a9c710b7e616683f194f18c43b43b869",
            "Name": "exactly prime",
            "Gas": 150,
            "NoBenchmark": false
        }
        ...
        test_cases.iter().for_each(|test| {
            let input = array_bytes::hex2bytes_unchecked(&test.input);
            let result = alt_bn128_multiplication_checked(&input).unwrap_err();
            assert_eq!(result, AltBn128Error::GroupError);
        });
    }

    #[test]
    fn alt_bn128_pairing_unchecked_test() {
        use serde_derive::Deserialize;
    ...
    #[test]
    fn alt_bn128_pairing_checked_test() {
        use serde_derive::Deserialize;
        ...
        test_cases.iter().for_each(|test| {
            let input = array_bytes::hex2bytes_unchecked(&test.input);
            let result = alt_bn128_pairing_checked(&input).unwrap_err();
            assert_eq!(result, AltBn128Error::GroupError);
        });
    }
}
